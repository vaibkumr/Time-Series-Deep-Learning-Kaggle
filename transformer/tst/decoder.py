...
# class MonthlyWeightedAvg(nn.Module):
#     """I wish I could do this with 1D convs, but I can't seem to figure out how. 
#         Update: It was easy, I did it."""
#     def __init__(self, n_inputs):
#         super(MonthlyWeightedAvg, self).__init__()
#         self.n_inputs = n_inputs
#         self.weights = nn.ParameterList([nn.Parameter(torch.randn(1)) for i in range(n_inputs)])

#     def forward(self, input):
#         assert self.n_inputs == input.shape[1]
#         past = []
#         for i in range(self.n_inputs):
#             past.append((input[:, i, :, :]* self.weights[i])[:, None, :, :])
#         res = torch.cat(past, 1).sum(1)    
#         return res/torch.Tensor(self.weights).sum()

class MonthlyWeightedAvg(nn.Module):
    def __init__(self, n_months):
        super(MonthlyWeightedAvg, self).__init__()
        self._conv = nn.Conv1d(n_months, 1, kernel_size=1, padding=0)   

    def forward(self, input):
        bs, t, nf = input.shape
        out = self._conv(input.reshape(bs, t//28, -1)).reshape(bs, 28, nf)     
        return out    
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self._mwa = MonthlyWeightedAvg(n_months)
        ...
        self._feedForward = PositionwiseFeedForward(d_model*2)

        self._layerNorm1 = nn.LayerNorm(d_model)
        self._layerNorm2 = nn.LayerNorm(d_model)
        # self._layerNorm3 = nn.LayerNorm(d_model)
        self._layerNorm3 = nn.LayerNorm(d_model*2)
    ...
    def forward(self, x: torch.Tensor, memory: torch.Tensor) -> torch.Tensor:
        ...
        residual = x
        _memory = self._mwa(memory)
        x = self._selfAttention(query=x, key=_memory, value=_memory)
        x = self._dopout(x)
        x = self._layerNorm2(x + residual)

        # Feed forward
        # x += _memory
        x = torch.cat([x, _memory], 2)
        residual = x
